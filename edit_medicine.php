...
$id = (int) ($_GET['id'] ?? 0);
$medicine = get_medicine_by_id($conn, $id);
if (!$medicine) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (!$errors && update_medicine($conn, $id, $data)) {
            header('Location: inventory.php?updated=1');
            exit;
        }
        if (!$errors) {
            $errors[] = 'Unable to update medicine.';
        }
    }
}
...
?>
<section class="page-header"><h1>Edit Medicine</h1></section>
...
<form class="panel" method="post" data-validate>
    ...
    <div class="form-grid">
        ...
        <div class="form-group"><label>Category</label><select name="category_id" required><?php foreach ($categories as $category): ?><option value="<?php echo h($category['id']); ?>" <?php echo (int) $data['category_id'] === (int) $category['id'] ? 'selected' : ''; ?>><?php echo h($category['name']); ?></option><?php endforeach; ?></select></div>
        <div class="form-group"><label>Supplier</label><select name="supplier_id" required><?php foreach ($suppliers as $supplier): ?><option value="<?php echo h($supplier['id']); ?>" <?php echo (int) $data['supplier_id'] === (int) $supplier['id'] ? 'selected' : ''; ?>><?php echo h($supplier['name']); ?></option><?php endforeach; ?></select></div>
        ...
        <div class="form-group"><label>Batch Number</label><input name="batch_number"></div>
    </div>
    <div class="actions"><button class="btn primary" type="submit">Update Medicine</button><a class="btn secondary" href="inventory.php">Cancel</a></div>
</form>
