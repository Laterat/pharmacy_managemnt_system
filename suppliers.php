    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'delete') {
            ...
            if (delete_supplier($conn, $id)) {
                ...
                $errors[] = 'Cannot delete supplier because they have related records.';
            }
        } else {
...
<button type="button" class="btn primary" id="showSupplierForm">
    <?php echo $editId ? 'Edit Supplier' : 'Add Supplier'; ?>
</button>
<div id="supplierFormContainer" style="display:<?php echo $editId ? 'block' : 'none'; ?>;">
    ...
    <form class="panel" method="post" data-validate>
        ...
        <div class="form-grid">
            <div class="form-group">
                <label>Name</label>
                <input name="name" required value="<?php echo h($data['name']); ?>">
            </div>

            <div class="form-group">
                <label>Contact Person</label>
                <input name="contact_person" value="<?php echo h($data['contact_person']); ?>">
            </div>

            <div class="form-group">
                <label>Phone</label>
                <input name="phone" value="<?php echo h($data['phone']); ?>">
            </div>
            ...
            <div class="form-group">
                <label>Address</label>
                <textarea name="address"><?php echo h($data['address']); ?></textarea>
            </div>
        </div>
        ...
        <tbody>
            <?php foreach ($suppliers as $supplier): ?>
                <tr>
                    ...
                    <td class="actions"><a class="btn secondary" href="suppliers.php?edit=<?php echo h($supplier['id']); ?>">Edit</a>
                        <form method="post">
                            <input type="hidden" name="csrf_token" value="<?php echo h(generate_csrf_token()); ?>">
                            <input type="hidden" name="action" value="delete">
                            <input type="hidden" name="id" value="<?php echo h($supplier['id']); ?>">
