...
$like = '%' . $search . '%';
$stmt = mysqli_prepare($conn, 'SELECT m.id, m.name, m.generic_name, m.price, m.stock_quantity, MIN(b.expiry_date) AS nearest_expiry FROM medicines m JOIN batches b ON b.medicine_id = m.id AND b.quantity > 0 WHERE m.stock_quantity > 0 AND (? = "" OR m.name LIKE ? OR m.generic_name LIKE ?) GROUP BY m.id ORDER BY m.name LIMIT 30');
